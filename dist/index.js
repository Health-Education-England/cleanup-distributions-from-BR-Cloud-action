...
class BrOperations {
    ...
    async getDistributionList(accessToken) {
        console.log('Get [Current] Distribution List from Bloomreach(BR) Cloud');
        const response = await this.apiClient.get('/v3/distributions', {
            headers: {
    ...
    async deleteDistributions(oldestDistributionIdsToBeDeleted, accessToken) {
        ...
        for (const distributionId of oldestDistributionIdsToBeDeleted) {
            await this.apiClient.delete('/v3/distributions/' + distributionId, {
                headers: {
                    Authorization: accessToken,
                }
            });
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const BrOperations_1 = __nccwpck_require__(8490);
const core = __importStar(__nccwpck_require__(2186));
const ApiClient_1 = __nccwpck_require__(3628);
...
async function cleanupDistributionsWhenBRCloudHitsMaxDistributions() {
    try {
        ...
        if (distributionList.count >= maxDistributions) {
            ...
            const sortedDistributionItems = distributionList.items.sort((a, b) => Date.parse(b.createdAt) - Date.parse(a.createdAt));
            ...
            const oldestDistributionIdsToBeDeleted = oldestDistributionsToBeDeleted.map((distribution) => distribution.id);
